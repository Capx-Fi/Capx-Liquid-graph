...
export function handleCreateVest(event: CreateVest): void {
  ...
  if(project != null){
    ...
    let userHoldingsID = generateID(generateID(_userAddress.toHexString(), _projectAddress),_unlockTime.toString());
...
export function handleTransferLock(event: TransferLock): void {
  ...
  if (_receiverAddress != Address.fromString("0x1f2aEf238D61Dc7b16A4aD78b54d70F262e14483")){
    let derivative = Derivative.load(_wrappedTokenAddress);
    if(derivative != null){
      // Creating a unique User ID, i.e. a combination of the User Address & the Wrapped Asset Address.
      let senderID = generateID(generateID(_userAddress.toHexString(), _wrappedTokenAddress),_unlockTime.toString());
      ...
      if(senderUserHoldings != null){
        ...
        // Creating a unique User ID, i.e. a combination of the User Address & the Wrapped Asset Address.
        let receiverID = generateID(generateID(_receiverAddress.toHexString(), _wrappedTokenAddress),_unlockTime.toString());
...
export function handleWithdraw(event: Withdraw): void {
  ...
  if(derivative != null){
    ...
    // Creating a unique User ID, i.e. a combination of the User Address & the Wrapped Asset Address.
    let userID = generateID(generateID(_userAddress.toHexString(), _wrappedTokenAddress), _unlockTime.toString());
